...
test("public repository contains no private calibration phrases or secret-looking keys",()=>{const text=files().filter(f=>!f.endsWith("test/safety.test.mjs")).map(f=>fs.readFileSync(f,"utf8")).join("\n");for(const pattern of[/>\$2k/i,/liquidat(?:ed|ion)/i,/ANTHROPIC_API_KEY\s*[:=]\s*["'][A-Za-z0-9_-]{10,}/i,/OPENAI_API_KEY\s*[:=]\s*["'][A-Za-z0-9_-]{10,}/i,/github_pat_[A-Za-z0-9_]+/i,/ghp_[A-Za-z0-9]+/i])assert.equal(pattern.test(text),false,`forbidden public content: ${pattern}`)});
