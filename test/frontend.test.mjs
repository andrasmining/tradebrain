...
test("unknown state is neutral, not green",()=>{assert.match(js,/status-neutral/);assert.match(js,/Awaiting first \$\{label\} assessment/);assert.match(js,/validated history remains visible in the comparison chart/);assert.doesNotMatch(js,/missing[^;\n]*green/i)});
...
test("AI switch is sticky, compact, and preserves viewport position",()=>{const selector=html.indexOf('class="ai-switch-sticky"'),timeline=html.indexOf('id="forecast-heading"');assert.ok(selector>0&&selector<timeline);assert.doesNotMatch(html,/Choose assessment provider/);assert.doesNotMatch(html,/Controls AI-specific sections/);assert.match(responsive,/\.ai-switch-sticky\s*\{[^}]*position:sticky/s);assert.match(js,/captureViewportAnchor/);assert.match(js,/restoreViewportAnchor/);assert.match(js,/window\.scrollBy/);assert.doesNotMatch(js,/scrollIntoView/)});
test("provider-specific sections show the active provider without verbose scope copy",()=>{assert.match(html,/data-active-ai/);assert.match(html,/BOTH AI/);assert.match(html,/ALL AI/);assert.doesNotMatch(html,/Switching changes the Risk timeline/);assert.match(responsive,/\.ai-section-chip/)});
