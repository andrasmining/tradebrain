...
if (!Array.isArray(history.items)) throw new Error("ChatGPT history.items must be an array");
...
for (const candidate of candidates) {
  ...
  let snapshot;
  try { snapshot=readJson(candidate.file); }
  catch(error){ if(indexed) throw new Error(`${candidate.repo} is indexed but contains invalid JSON: ${error.message}`); rejected.push(`${candidate.repo}: invalid JSON: ${error.message}`); continue; }
  const errors=validateStatus(snapshot,provider);
  if(!indexed) errors.push(...validateStatusContract(snapshot));
  if(errors.length){ if(indexed) throw new Error(`${candidate.repo} is indexed but invalid: ${errors.join("; ")}`); rejected.push(`${candidate.repo}: ${errors.join("; ")}`); continue; }
  if(knownGeneratedAt.has(snapshot.generatedAt)||indexed) continue;
  history.items.push(historyItem(snapshot,candidate.repo)); knownGeneratedAt.add(snapshot.generatedAt); knownSnapshots.add(candidate.repo); changed=true;
}
...
if(!history.items.length) throw new Error("ChatGPT history contains no items");
const latestItem=history.items.at(-1),latestSnapshotFile=path.join(root,latestItem.snapshot);
...
const latestErrors=[...validateStatus(latestSnapshot,provider),...validateStatusContract(latestSnapshot)];
...
if(!sameJson(existingStatus,latestSnapshot)){fs.copyFileSync(latestSnapshotFile,statusFile);changed=true;}
if(!sameJson(existingSignal,expectedSignal)){fs.writeFileSync(signalFile,`${JSON.stringify(expectedSignal,null,2)}\n`);changed=true;}
