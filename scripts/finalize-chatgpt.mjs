...
if (!fs.existsSync(historyFile)) {
  throw new Error("providers/chatgpt/history.json is missing");
}
...
for (const candidate of candidates) {
  ...
  if (errors.length) {
    if (indexed) {
      throw new Error(`${candidate.repo} is indexed but invalid: ${errors.join("; ")}`);
    }
...
if (rejected.length) {
  console.warn(`Rejected ${rejected.length} invalid unindexed ChatGPT snapshot(s); continuing with valid snapshots:`);
...
console.log(changed
  ? `Finalized ChatGPT publication at ${latestSnapshot.generatedAt}.`
  : `ChatGPT publication already coherent at ${latestSnapshot.generatedAt}.`);
