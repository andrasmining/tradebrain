...
Each section fails independently. A partially successful run publishes surviving sections with warnings and `collectionStatus: "partial"`. A total collection failure exits nonzero and preserves the previous good `latest.json`; consumers use its `generatedAt` age to recognize a stale collection. The Finviz evidence contract introduced in v1.2.0 and inherited by active ChatGPT v1.3.0 treats a collection up to 90 minutes old as reasonably fresh, but continues independent research for stale, partial, malformed, or unavailable context; that collection freshness does not turn unknown-as-of daily performance into a current intraday observation.
...
`prompts/chatgpt/v1.3.0.md` is the explicitly confirmed active scheduled ChatGPT contract. It inherits `prompts/chatgpt/v1.2.0.md`, which inherits `prompts/chatgpt/v1.1.1.md` and `prompts/chatgpt/v1.1.0.md`. The inherited snapshot-only publication and optional Finviz evidence rules remain in force, while v1.3.0 additionally requires bounded provider-authored analysis, drivers, and verified already-known news on every one of the 24 forecast hours. That activation was an explicit external scheduler migration, not a side effect of adding the repository file.
...
`prompts/claude/v1.3.0.md` is available for a future explicit Claude migration and adds the same rich 24-hour forecast contract. Its presence does not establish Claude's active external scheduler version; that must still be verified separately.

## Dashboard hierarchy, comparison, and freshness

The dashboard's Level-1 overview is manifest-driven and presents two cross-provider surfaces in this order:

1. a multi-provider 24-hour risk timeline with an always-visible exact-hour comparison and one full analysis card per provider directly underneath;
2. a Historical risk trend built from audit history and fresh published forecasts.

Every enabled entry in `config/providers.json` joins these views in manifest order, including future providers. The timeline aligns exact published hourly slots, keeps every provider's actual risk-status color, and represents stale, missing, or invalid values neutrally. Its current UTC clock-hour is selected on first load. A manual hour selection survives minute refreshes while that timestamp remains in the rolling 24-hour window, then falls back to the new current hour. Selection is session-only and never creates a combined action.

The selected-hour comparison uses providers as aligned desktop rows and responsive mobile mini-cards. Each provider analysis comes from that exact `forecast[].ts`; matching scheduled events use the half-open interval `[slot.ts, slot.ts + 1h)`. Rich v1.3 publications expose canonical hour-specific analysis, drivers, and verified already-known news. Providers still publishing an older contract use an exact matching legacy `forecastDetail[].comment` for the first six slots and otherwise state that detailed hourly analysis was not published by that provider version. The UI never fabricates reasoning or substitutes the provider's overall assessment.

Any agreement or divergence wording is display-only metadata across the fresh available providers. TradeBrain does not average scores or synthesize a consensus trading signal. Below the Historical risk trend, the provider selector controls only the Level-2 detail sections: Current assessment, Next 24 hours, Upcoming events, and Provider history. The Next-24 view is a contained, native-scroll card rail preserving the selected provider's exact 24 published timestamps; overlapping Level-1 hours reuse the same canonical forecast-hour objects as the top analysis. Changing the provider selection does not alter or reset the Level-1 views.
...
The Historical risk trend renders one display-only combined chart from provider audit history and current provider-published `forecast[]` data. Tail/Kill, Stress/DD, and Confidence are independent toggles, so any subset can overlap for the enabled providers on the same fixed 0-100% scale. The true first-use default enables Tail and Stress while leaving Confidence off; an explicitly saved user selection still wins. The chart-range selector offers 1, 3, 7, 14, or 30 total days and defaults to three: 48 hours of history plus 24 published hourly slots per provider. One day is forecast-only. The history and forecast panes use explicitly labeled separate time scales so the forecast remains readable on mobile at longer ranges; lines never connect across their divider. Exact provider forecast timestamps are preserved, so the shared forecast pane may span more than 24 elapsed hours when provider origins differ. Forecasts are shown only from complete, fresh current provider state and are never synthesized or extrapolated from history.
