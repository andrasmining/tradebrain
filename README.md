...
`prompts/chatgpt/v1.3.0.md` and `prompts/claude/v1.3.0.md` are staged contracts for review, not active scheduler migrations. They preserve the existing risk, exact-hour, Finviz, ownership, safety, and snapshot-only rules while adding bounded provider-authored analysis, drivers, and verified already-known news to every forecast hour. ChatGPT's confirmed active scheduler remains on v1.2.0; Claude's active version must still be verified externally.
...
The selected-hour comparison uses providers as aligned desktop rows and responsive mobile mini-cards. Each provider analysis comes from that exact `forecast[].ts`; matching scheduled events use the half-open interval `[slot.ts, slot.ts + 1h)`. Rich v1.3 publications expose canonical hour-specific analysis, drivers, and verified already-known news. Before scheduler migration, the UI uses an exact matching legacy `forecastDetail[].comment` for the first six slots and otherwise states that detailed hourly analysis was not published by that provider version. It never fabricates reasoning or substitutes the provider's overall assessment.
